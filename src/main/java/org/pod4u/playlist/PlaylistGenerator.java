        ...
        try {
            ...
            if (!otherAudio.isEmpty()) {
                ...
                for (int i=0; i+50-otherAudio.size()<50; i+=50) {
                    //HttpRequest request = HttpRequest.newBuilder().uri(new URI("https://api.freqblog.com/v1/audio-features?ids=" + Arrays.toString(mainIds).replace("[", "").replace("]", "").replace(", ", ",").replace("\"", ""))).header("Accept", "application/json").header("X-Api-Key", "sk_live_d5867bf3ccbda0abbe32d63549af8357308cf801e6efbef8").GET().build();
